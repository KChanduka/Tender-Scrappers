...
async function SA_PortAdelaideEnfield() {
  try {
    ...
    const pageUrls = [
      "https://www.tenders.sa.gov.au/tender/search?keywords=&tenderCode=&tenderState=OPEN&tenderType=&issuingBusinessId=80203&awardedSupplier.id=&awardedSupplier.name=&openThisWeek=false&openingDateFrom=&openingDateTo=&closeThisWeek=false&closingDateFrom=&closingDateTo=&groupBy=NONE&page=&searchTitle=",
    ];
    ...
    for (const dataItem of allData) {
      ...
      const location = ["SA"];
      const region = ["City of Port Adelaide Enfield"];
    ...
    await reArrangeTheOldAndNewData(tenderData, "sa-pae-");
...
SA_PortAdelaideEnfield();
module.exports = SA_PortAdelaideEnfield;
